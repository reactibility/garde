...
class ModuleSpec extends TestKit(ActorSystem("test", ModuleSpec.Config))
  ...
  override protected def beforeAll() {
    ...
    sup = system.actorOf(Props(new ModuleSupervisor()), "sup")
  }
  ...
  "Module creation" must {
    ...
    "succeed" in {
      ...
      sup ! GetSupervisorState
      expectMsg(timeout.duration, ModuleSupervisorState(Seq("module-1")))
      val module = Await.result(system.actorSelection("/user/sup/module-1").resolveOne(), timeout.duration)
      module ! GetState
      expectMsg(timeout.duration, ModuleState("module-1", 0L, "Module1"))
    }

    "fail on duplication creation" in {
      sup ! CreateModule("module-1", "Module1")
      expectMsg(timeout.duration, Failure(NonEmptyList("Module module-1 already exists.")))
    }
  ...
  "Module recovery" must {
    ...
    "succeed" in {
      ...
      sup ! Recover
      sup ! GetSupervisorState
      expectMsg(timeout.duration, ModuleSupervisorState(Seq("module-1", "module-2", "module-3")))

      Thread.sleep(2000)
      val m1 = Await.result(system.actorSelection("/user/sup/module-1").resolveOne(), timeout.duration)
      m1 ! GetState
      expectMsg(timeout.duration, ModuleState("module-1", 1L, "New Name"))

//      val m2 = Await.result(system.actorSelection("/user/sup/module-2").resolveOne(), timeout.duration)
//      m2 ! GetState
//      expectMsg(timeout.duration, ModuleState("module-2", 0L, "Module2"))
    }
  }
}
